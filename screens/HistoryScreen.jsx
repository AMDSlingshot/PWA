...
export default function HistoryScreen() {
  ...
  async function handleExport(session) {
    try {
      ...
      Alert.alert(
        "Export",
        JSON.stringify(JSON.parse(json), null, 2).slice(0, 500) +
          "\n\n[Copy from console]",
      );
  ...
  function renderSession({ item: session }) {
    ...
    return (
      <View
        style={[styles.sessionCard, isRecording && styles.sessionCardRecording]}
      >
        {/* Header row */}
        <View style={styles.cardHeader}>
          ...
          {!isRecording && avgIRI != null && (
            <View
              style={[
                styles.iriChip,
                {
                  borderColor: iriColor + "60",
                  backgroundColor: iriColor + "15",
                },
              ]}
            >
              ...
              <Text style={[styles.iriChipLabel, { color: iriColor + "CC" }]}>
                {iriLabel}
              </Text>
            </View>
          )}
          {!isRecording && avgIRI == null && (
            ...
            >
              ...
              <Text style={[styles.iriChipLabel, { color: COLORS.textMuted }]}>
                NO IRI
              </Text>
            </View>
          )}
        ...
        <View style={styles.statsRow}>
          <StatChip
            label="DISTANCE"
            value={formatDistance(session.distance_m)}
          />
          <StatChip
            label="SEGMENTS"
            value={String(session.segment_count || 0)}
          />
          <StatChip
            label="DURATION"
          ...
          {stats && stats.segment_count > 0 && (
            <StatChip
              label="MAX IRI"
              value={stats.max_iri != null ? stats.max_iri.toFixed(1) : "—"}
            />
          )}
          {(!stats || !stats.segment_count) && (
            <StatChip label="STATUS" value={isRecording ? "LIVE" : "✓ DONE"} />
          )}
        </View>
      ...
      {isLoading ? (
        ...
        <View style={styles.centered}>
          ...
          <Text style={styles.emptySubtitle}>
            Start a recording to collect road data
          </Text>
        </View>
      ) : (
        <FlatList
